...
export function RelativeFrameSelector({ value, onChange }: { value: RuntimeRelativeFrame; onChange: (value: RuntimeRelativeFrame) => void }) {
  return (
    <label className="block">
      ...
      <select value={value} onChange={(event) => onChange(event.target.value as RuntimeRelativeFrame)} className="mt-1 w-full border border-cyan-300/25 bg-black/45 px-3 py-2 text-sm text-zinc-100 outline-none focus:border-cyan-300">
