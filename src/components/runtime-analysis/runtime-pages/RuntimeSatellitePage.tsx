...
export function RuntimeSatellitePage({ onResult, onLoadingChange, onLog, onPrimaryNoradChange }: RuntimePageProps) {
  ...
  const run = async () => {
    const norad = Number(noradCatalogId);
  ...
  return (
    <div className="space-y-4">
      ...
      <button type="button" onClick={run} disabled={loading} className="w-full border border-cyan-300 bg-cyan-300 px-4 py-3 font-mono text-xs font-semibold uppercase tracking-[0.16em] text-slate-950 transition hover:bg-cyan-200 disabled:opacity-50">{loading ? "Loading" : "Load Satellite"}</button>
      {result && (
        <>
          <ResultSummary items={[
